...
import com.lwd.jobportal.pricing.FeatureAccessService;
import com.lwd.jobportal.repository.*;
import com.lwd.jobportal.searchhistory.SaveSearchHistoryRequest;
import com.lwd.jobportal.searchhistory.SearchHistory;
...
public class JobService {
    ...
    // ==================================================
    // CREATE JOB BY RECRUITER / COMPANY_ADMIN
    // ==================================================
    @Transactional
    public JobResponse createJobAsRecruiter(CreateJobRequest request) {
    	...
    	if (!Boolean.TRUE.equals(company.getVerified())) {
    		throw new IllegalStateException("Company is not verified");
    	}
    	
    	if (request.getApplicationSource() == ApplicationSource.EXTERNAL &&
    			(request.getExternalApplicationUrl() == null ||
    			request.getExternalApplicationUrl().isBlank())) {
    		throw new IllegalArgumentException(
    				"External application URL is required for EXTERNAL jobs"
    				);
    	}
    ...
    public PagedJobResponse getMyJobs(int page) {
    	...
    	// ================= COMPANY_ADMIN =================
    		...
    		jobPageRaw = jobRepository.findByCompanyId(company.getId(), pageable);
    	}
    	// ================= RECRUITER =================
    	else if (role == Role.RECRUITER) {
    		
    		jobPageRaw = jobRepository.findByCreatedById(userId, pageable);
    	}
    	// ================= INVALID =================
    	else {
    ...
    public PagedJobResponse searchJobsByRole(JobSearchRequest request, int page) {
        ...
        List<Long> jobIds = jobPage.getContent().stream()
                .map(Job::getId)
                .toList();
        ...
        Page<JobResponse> responsePage = jobPage.map(job -> mapToResponse(job, countMap));
 ...
 // ================= ROLE BASED VALIDATION =================
    private void validateOwnership(User user, Job job) {
     ...
     // COMPANY_ADMIN -> jobs of their company
        if (user.getRole() == Role.COMPANY_ADMIN) {
            if (job.getCompany() == null ||
                job.getCompany().getCreatedBy() == null ||
                !job.getCompany().getCreatedBy().getId().equals(user.getId())) {
                throw new IllegalArgumentException("You can only modify jobs of your company");
            }
            return;
        }
