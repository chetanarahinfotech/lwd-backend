...
public class PlanService {
    ...
    public List<RecruiterPlanResponse> getCompanyAdminPlans() {
        List<Plan> plans = planRepository
                .findByTypeAndIsActiveTrueOrderByPriceAsc(PlanType.COMPANY_ADMIN);

        log.info("Fetched {} recruiter admin plans", plans.size());

        return plans.stream()
                .map(this::toCompanyAdminPlanResponse)
                .toList();
    }
    ...
    private RecruiterPlanResponse toCompanyAdminPlanResponse(Plan plan) {
        return RecruiterPlanResponse.builder()
                .planId(plan.getId())
                .planName(plan.getName())
                .price(plan.getPrice())
                ...
                // SAME PLAN TYPE but admin may see more features
                .features(safeMapFeatures(plan.getId(), PlanType.COMPANY_ADMIN))
